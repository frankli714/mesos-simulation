...
is NP-hard, and consequently that there is \emph{no} non-trivial policy that is Pareto efficient);
...
Naturally, it is much easier to prove guarantees for the exact version: when the scheduling problems are small or are broken up into small pieces,
this is not a significant distortion. Moreover, we can never hope for anything beyond this kind of approximate
Pareto efficiency for computational reasons, so this seems to be the theoretical property of most interest.
