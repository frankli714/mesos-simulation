...
Specifically, the auction tries to fit bids into the available resources in the
cluster as to maximize the amount of revenue from the auction. With each task
package, we associate a \emph{current price} in addition to the willingness to
pay; then the difference between the latter and the former is  the The agent
representing a framework's bid computes 
